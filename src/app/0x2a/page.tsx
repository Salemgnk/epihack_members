...
export default function SecretLoginPage() {
    ...
    useEffect(() => {
        const handleKeyPress = (e: KeyboardEvent) => {
            ...
            const konamiString = newSequence.join(',');
            const targetString = KONAMI_CODE.join(',');

            if (konamiString === targetString) {
                setKonamiActivated(true);
                setShowHint(false);
                // Play success sound or visual effect
                const audio = new Audio('data:audio/wav;base64,UklGRnoGAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YQoGAACBhYqFbF1fdJivrJBhNjVgodDbq2EcBj+a2/LDciUFLIHO8tiJNwgZaLvt559NEAxQp+PwtmMcBjiR1/LMeSwFJHfH8N2QQAoUXrTp66hVFApGn+DyvmwhBSuBzvLZiTQHHGi557Zk');
                audio.volume = 0.3;
                audio.play().catch(() => { });
            }
        };
        ...
        <div className="min-h-screen bg-black text-white flex items-center justify-center p-4 relative overflow-hidden">
            ...
            {konamiActivated && (
                <div className="absolute inset-0 pointer-events-none">
                    <div className="absolute inset-0 bg-system-green/5 animate-pulse" />
                    <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-system-green to-transparent animate-pulse" />
                    <div className="absolute bottom-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-system-green to-transparent animate-pulse" />
                </div>
            )}
            ...
            <div className="relative z-10 w-full max-w-md">
                <div className="text-center mb-8">
                    ...
                    <p className="text-gray-400 text-sm tracking-wider uppercase">0x2A Access Portal</p>
                    {konamiActivated && (
                        <div className="mt-2 inline-flex items-center gap-2 px-3 py-1 bg-system-green/10 border border-system-green/30 rounded-full">
                            <Sparkles className="w-4 h-4 text-system-green" />
                            <span className="text-xs text-system-green font-tech">ACCESS GRANTED</span>
                        </div>
                    )}
                </div>
                ...
                <div className="bg-gray-900/50 backdrop-blur-xl border border-gray-800 rounded-lg p-8 shadow-2xl">
                    ...
                    {/* Fake button (always visible, rickrolls) */}
                    {!konamiActivated && (
                        <button
                            onClick={handleFakeLogin}
                            disabled={loading}
                            className="w-full group relative overflow-hidden rounded-lg bg-gradient-to-r from-red-600 to-orange-600 p-[2px] transition-all hover:scale-[1.02] disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 mb-4"
                        >
                    ...
                    {/* Real button (only visible after Konami code) */}
                    {konamiActivated && (
                        <button
                            onClick={handleRealLogin}
                            disabled={loading}
                            className="w-full group relative overflow-hidden rounded-lg bg-gradient-to-r from-green-600 to-blue-600 p-[2px] transition-all hover:scale-[1.02] disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 animate-fade-in-up"
                        >
                            <div className="relative bg-gray-900 rounded-lg px-6 py-4 transition-all group-hover:bg-transparent">
                                <div className="flex items-center justify-center gap-3">
                                    {loading ? (
                                        ...
                                        <>
                                            ...
                                            <span className="font-semibold">✨ Authentification Réelle ✨</span>
                                        </>
                                    )}
                                </div>
                            </div>
                ...
                <div className="mt-6 text-center">
                    <div className="inline-flex items-center gap-2 text-xs text-gray-500">
                        ...
                        <span>Connexion {konamiActivated ? 'authentique' : 'sécurisée'} via Discord OAuth</span>
                    </div>
                </div>

                {!konamiActivated && (
                    <p className="mt-4 text-center text-xs text-muted-foreground/50 font-tech">
